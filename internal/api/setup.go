...
func (s *Server) handleSetup(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(b, "| `session_id` | string | no | User-session identifier; generate once per visit and reuse |\n")
	fmt.Fprintf(b, "| `user_id` | string | no | Stable identifier for a logged-in user |\n")
	fmt.Fprintf(b, "| `timestamp` | ISO 8601 string | no | Defaults to server receive time if omitted |\n")
	fmt.Fprintf(b, "| `url` | string | no | Page URL |\n")
	fmt.Fprintf(b, "| `referrer` | string | no | Referring URL |\n")
	fmt.Fprintf(b, "| `properties` | object | no | Arbitrary key/value pairs |\n\n")
	...
	fmt.Fprintf(b, "# or load from CDN (auto-tracks page views on init):\n")
	...
	fmt.Fprintf(b, "  \"reason\":   \"SPLIT\",\n")
	fmt.Fprintf(b, "  \"error_code\": \"\"\n")
	fmt.Fprintf(b, "}\n")
	fmt.Fprintf(b, "```\n\n")
	fmt.Fprintf(b, "`reason` is one of: `SPLIT` (bucketed), `TARGETING_MATCH` (a rule matched),\n")
	fmt.Fprintf(b, "`DISABLED` (flag paused), `ERROR` (with `error_code: FLAG_NOT_FOUND` etc).\n\n")
