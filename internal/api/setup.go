...
func (s *Server) handleSetup(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(b, "| `default_value` | any | no | Returned when the flag is missing or eval errors. Also the value the flag is auto-registered with — see below. |\n")
	fmt.Fprintf(b, "| `context` | object | no | User/session attributes for targeting + bucketing |\n")
	fmt.Fprintf(b, "| `kind` | string | no | `experiment` (default) or `config`. Honoured only when this call auto-registers the flag. |\n\n")
	...
	fmt.Fprintf(b, "  \"cache_max_age_seconds\": 0,\n")
	...
	fmt.Fprintf(b, "- `STATIC` — a **config** flag: one value for everyone, no bucketing. See below.\n")
	fmt.Fprintf(b, "- `DISABLED` — the flag is paused or not yet activated. You get **the flag's own default variant value**, not the `default_value` you sent. The two differ, and the difference is the point: an auto-registered flag is configurable from the dashboard before anyone activates it.\n")
	fmt.Fprintf(b, "- `ERROR` — something went wrong; check `error_code` (e.g. `FLAG_NOT_FOUND`, `AUTO_REGISTER_LIMIT`). `value` echoes back the `default_value` you supplied.\n\n")
	...
	fmt.Fprintf(b, "`cache_max_age_seconds` is how long you may reuse this result without\n")
	fmt.Fprintf(b, "re-evaluating; the same value is sent as `Cache-Control`. It is the one\n")
	fmt.Fprintf(b, "sanctioned polling interval, so every consumer doesn't invent its own.\n")
	fmt.Fprintf(b, "`0` means do not cache — an experiment resolves per targeting key and every\n")
	fmt.Fprintf(b, "read is a data point, so caching one would both mis-bucket and silently\n")
	fmt.Fprintf(b, "drop the analytics.\n\n")
	...
	writeFlagKindsSection(b, s.flagAutoRegisterMax)
	...
	fmt.Fprintf(b, "```\n\n")
	fmt.Fprintf(b, "### Go\n\n")
	fmt.Fprintf(b, "The Go SDK has typed helpers, so you don't hand-roll the client:\n\n")
	fmt.Fprintf(b, "```go\n")
	fmt.Fprintf(b, "import funnelbarn \"github.com/webwiebe/funnelbarn/sdks/go\"\n\n")
	fmt.Fprintf(b, "funnelbarn.Init(funnelbarn.Options{\n")
	fmt.Fprintf(b, "    APIKey:   %q,\n", plaintext)
	fmt.Fprintf(b, "    Endpoint: %q,\n", publicURL)
	fmt.Fprintf(b, "    ProjectName: %q,\n", slug)
	fmt.Fprintf(b, "})\n\n")
	fmt.Fprintf(b, "// Returns your default on any failure — never an error you can ignore.\n")
	fmt.Fprintf(b, "if funnelbarn.EvaluateBool(ctx, \"checkout_redesign\", false,\n")
	fmt.Fprintf(b, "    map[string]any{\"targeting_key\": userID}) {\n")
	fmt.Fprintf(b, "    showNewCheckout()\n")
	fmt.Fprintf(b, "}\n\n")
	fmt.Fprintf(b, "// Config values come back typed, and are cached for the interval the\n")
