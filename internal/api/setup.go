...
func (s *Server) handleSetup(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(b, "Every request to `/api/v1/events` must include both:\n\n")
	fmt.Fprintf(b, "- `%s: <key>` — authenticates the request.\n", keyHeader)
	fmt.Fprintf(b, "- `%s: %s` — routes the event to your project. Use the **slug** (not the project ID).\n\n", projHeader, slug)
	fmt.Fprintf(b, "Both headers are case-insensitive. The slug-in-the-body trick that some\nanalytics products accept is **not** supported here — funnelbarn reads project\nrouting from the header only.\n\n---\n\n")
