...
// neither the request nor the server's dependencies.
func buildSetupMarkdown(project repository.Project, apiKey, publicURL string, flagAutoRegisterMax int, mcpEnabled bool) string {

	baseURL := publicURL
	ingestURL := publicURL + "/api/v1/events"
	sdkURL := publicURL + "/sdk.js"
	setupURL := publicURL + "/api/v1/setup/" + project.Slug
	...
	fmt.Fprintf(b, "> The API key above is scoped to **ingest** only (event tracking). It is deterministic and will be identical on every visit. No apiKey is stored server-side.\n\n")
	...
	fmt.Fprintf(b, "> Project routing reads `%s` only — do not put the project.Slug in the request body.\n\n---\n\n", projHeader)
	...
	fmt.Fprintf(b, "- **Password fields** are automatically masked as `***` — no apiKey credentials captured.\n")
	...
	writeFlagKindsSection(b, flagAutoRegisterMax)
	...
	writeFlagAPISection(b, publicURL, project.Slug)
